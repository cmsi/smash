...
&                 w9(16,j,igrid))*tval2+w9(17,j,igrid))*tval2+w9(18,j,igrid)
        ...
        case default
          write(iout,'("Subroutine rysroot supports up to nroots=9")')
          call iabort
      end select
      return
end
...
subroutine rys(t8,norder,ab8,w8)
  integer :: norder
  integer :: i, j
  real*16 t,fmt(0:100),a(norder,norder),b(norder),x(norder),ab(norder),w(norder),tmp(norder,norder),y(norder)
  real*16 tmp2(norder)
  real*8 t8, ab8(*),w8(*)
!
  t=t8
  do i=0,2*norder-1
    call calcfmt2(fmt(i),t,i)
  enddo
!
...
do k=0,norder-1
  do i=1,norder
    do j=1,norder
      a(norder-i+1,norder-j+1)=fmt(i+j-2)
    enddo
    b(norder-i+1)=-fmt(norder+i-1)
  enddo
!
...
!
  do i=1,norder
    do j=1,norder
      tmp(i,mod(k+j-1,norder)+1)=a(i,j)
    enddo
!   tmp2(mod(k+i-1,norder)+1)=b(i)
  enddo
  do i=1,norder
    do j=1,norder
      a(i,j)=tmp(i,j)
...
enddo
  do i=1,norder
    tmp(norder-i+1,1)=ab(i)
  enddo
  do i=1,norder
    ab(i)=tmp(i,1)
  enddo
  do i=1,norder
    tmp(norder-i+1,1)=w(i)
  enddo
  do i=1,norder
    w(i)=tmp(i,1)
  enddo
...
subroutine calcfmt2(fmt,t,m)
  implicit none
  real*16 t,t2,expt,fmt,a,rparam(0:801)
  real*16,parameter::zero=0.0Q+00, one=1.0Q+00, two=2.0Q+00,cutoff=1.0Q-29
  integer i,m,n
  ...
  if(t.eq.zero) then
    ...
    do i=m,400
      a=a*t2*rparam(2*i+1)
      fmt=fmt+expt*a
      if(i.gt.50.and.expt*a/fmt.lt.cutoff)exit
      if(i.eq.400) stop
    enddo
  endif
end
...
subroutine simul(a,b,x,ndim)
  implicit none
  integer :: ndim, i, j, k
  real*16 :: a(ndim,ndim),b(ndim),x(ndim),val
...
subroutine newton(c,x,t,norder)
  implicit none
  integer :: norder, i, j, k
  real*16 :: c(norder),x(norder),t,xold,fx,dfx
  real*16 :: r12=2.7525q-01,r22=2.7247q+00
  real*16 :: r13=1.9016q-01,r23=1.7845q+00,r33=5.5242q+00
!
  ...
  do i=1,norder
    do j=1,1000
      ...
      do k=1,norder
        fx=fx+c(k)*xold**(k-1)
      enddo
      dfx=norder*xold**(norder-1)
      do k=2,norder
        dfx=dfx+(k-1)*c(k)*xold**(k-2)
      enddo
      x(i)=x(i)-fx/dfx
      if(abs((x(i)-xold)/x(i)).lt.1.0q-28)exit
      if(j.eq.1000)stop
    enddo
  enddo
end
