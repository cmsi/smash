...
!
      ...
      use basis, only : locatom, locprim, mprim, mbf, mtype, ex, coeff
...
!---------------------------------------
  subroutine calcexchange(xint,maxdim)
!---------------------------------------
!
! Driver of exchange integral calculation
!
! Out : xint (exchange integral)
!
      use procpar, only : nproc, myrank, MPI_SUM, MPI_COMM_WORLD
      use basis, only : nshell, mtype, mbf
      use thresh, only : cutint2, threshex
      implicit none
...
!
      call zeroclr(xint,nshell*(nshell+1)/2)
!
...
!$OMP do
        do jsh = 1,ish
          ...
          enddo
          ij= ii+jsh
          xint(ij)=sqrt(xintmax)
        enddo
!$OMP end do
      enddo
!$OMP end parallel
!
      call para_allreduce(xint,xintmax,nshell*(nshell+1)/2,"D",MPI_SUM,MPI_COMM_WORLD,1)
!
      cutint2= cutsave
