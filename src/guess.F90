...
!
      if(guess == 'HUCKEL') then
        ...
        call huckelguess(cmoa,overinv,nproc2,myrank2,mpi_comm2,datacomp)
        if(master) write(*,*)
...
!
      elseif(guess == 'UPDATE') then
        call updatemo(cmoa,overinv,nproc2,myrank2,mpi_comm2,datacomp)
        if(scftype == 'UHF') call dcopy(nao*nao,cmoa,1,cmob,1)
!
      elseif(guess == 'CHECK') then
        if(master) write(*,'("   Guess MOs are read from checkpoint file and projected.",/)')
        call checkguess(cmoa,cmob,overinv,nproc2,myrank2,mpi_comm1,mpi_comm2,datacomp)
      elseif(guess == 'HCORE') then
        if(master) &
&         write(*,'("   Guess MOs are generated by diagonalizing core Hamiltonian matrix.",/)')
        call hcoreguess(cmoa,h1mtrx,ortho,overinv,nproc2,myrank2,mpi_comm2,datacomp)
        if(scftype == 'UHF') call dcopy(nao*nao,cmoa,1,cmob,1)
      else
