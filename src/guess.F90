...
!
      ...
      real(8),intent(in) :: h1mtrx(databasis%nao*(databasis%nao+1)/2), ortho(databasis%nao**2)
...
!
        case('DFTB')
          if(datacomp%master) &
&           write(datacomp%iout,'("   Guess MOs are generated using DFTB orbitals.")')
          call dftbguess(cmoa,overinv,datajob,datamol,databasis,datacomp)
          if(datacomp%master) write(datacomp%iout,*)
          if(datajob%scftype == 'UHF') call dcopy(databasis%nao**2,cmoa,1,cmob,1)
!
        case('UPDATE')
          call updatemo(cmoa,overinv,datajob,datamol,databasis,datacomp)
          if(datajob%scftype == 'UHF') call dcopy(databasis%nao**2,cmoa,1,cmob,1)
!
        case('CHECK')
          if(datacomp%master) &
&           write(datacomp%iout,'("   Guess MOs are read from checkpoint file and projected.",/)')
          call checkguess(cmoa,cmob,overinv,datajob,datamol,databasis,datacomp)
!
        case('HCORE')
          if(datacomp%master) write(datacomp%iout, &
&           '("   Guess MOs are generated by diagonalizing core Hamiltonian matrix.",/)')
          call hcoreguess(cmoa,h1mtrx,ortho,overinv,databasis%nao,datamol%nmo,datacomp)
          if(datajob%scftype == 'UHF') call dcopy(databasis%nao**2,cmoa,1,cmob,1)
        case default
          if(datacomp%master) &
...
!
      if(datacomp%master) then
        write(datacomp%iout,'(" If the calculation stops at the beginning of SCF, " &
&                 "increase the stack size.")')
        write(datacomp%iout,'(" bash: Write the following two lines in the ~/.bashrc file.")')
        write(datacomp%iout,'("       ulimit -s unlimited")')
        write(datacomp%iout,'("       export OMP_STACKSIZE=1G")')
        write(datacomp%iout,'(" tcsh: Write the following two lines in the ~/.tcshrc file.")')
        write(datacomp%iout,'("       unlimit")')
        write(datacomp%iout,'("       setenv OMP_STACKSIZE 1G",/)')
      endif
      return
end
...
!------------------------------------------------------------------------------
  subroutine checkguess(cmoa,cmob,overinv,datajob,datamol,databasis,datacomp)
!------------------------------------------------------------------------------
!
! Read and project MOs
!
...
!
      ndim= max(nao_g,nao)
      call memset(nao_g*(nao*2+ndim+nao_g+1),datacomp)
...
! Unset arrays
!
      call memunset(nao_g*(nao*2+ndim+nao_g+1),datacomp)
...
!
      ...
      integer :: nao3, idis(datacomp%nproc2,14), isize
...
!
      nao3= nao*(nao+1)/2
      call distarray(idis,nmo,nao,nao3,nao,nao,nao,nao,datacomp%nproc2)
      isize= idis(datacomp%myrank2+1,3)
!
...
!
      dftb0=zero
      call formdftb0(dftb0,overlap,eigen,coord_g,nao_v,nshell_v,datajob%threshex, &
&                    datamol,dataguessbs)
!
...
!
      ...
      enddo
!
      deallocate(dftb0,dftb1,gamma12, &
&                uhub,qmulliken)
      call memunset(2*nao_g*nao_g+nshell_v*nshell_v+3*nshell_v,datacomp)
!
      return
end
...
!-------------------------------------------------------------------------------
  subroutine formdftb0(dftb0,overlap,energy,coord_g,nao_v,nshell_v,threshex, &
&                      datamol,dataguessbs)
!-------------------------------------------------------------------------------
!
