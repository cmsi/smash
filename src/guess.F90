...
!
!------------------------------------------------------------------------
  subroutine guessmo(cmoa,cmob,overinv,h1mtrx,ortho,datacomp)
!------------------------------------------------------------------------
!
...
!
      elseif(guess == 'UPDATE') then
        call updatemo(cmoa,overinv,datacomp%nproc2,datacomp%myrank2,datacomp%mpi_comm2,datacomp)
        if(scftype == 'UHF') call dcopy(nao*nao,cmoa,1,cmob,1)
!
      elseif(guess == 'CHECK') then
        if(datacomp%master) &
&         write(*,'("   Guess MOs are read from checkpoint file and projected.",/)')
        call checkguess(cmoa,cmob,overinv,datacomp%nproc2,datacomp%myrank2,datacomp%mpi_comm2,datacomp)
      elseif(guess == 'HCORE') then
        if(datacomp%master) &
&         write(*,'("   Guess MOs are generated by diagonalizing core Hamiltonian matrix.",/)')
        call hcoreguess(cmoa,h1mtrx,ortho,overinv,datacomp%nproc2,datacomp%myrank2,datacomp%mpi_comm2,datacomp)
        if(scftype == 'UHF') call dcopy(nao*nao,cmoa,1,cmob,1)
      else
        if(datacomp%master) &
&         write(*,'(" Error! This program does not support guess= ",a10,"!")') guess
        call iabort
      endif
!
      if(datacomp%master) then
...
!
      nao_g2= nao_g*nao_g
      call memset(2*nao_g2+2*nao*nao_g+nao_g,datacomp)
...
! Set ionization potentials
!
      call huckelip(eigen,2,datacomp)
!
! Form extended Huckel matrix
!
...
!------------------------------------
  subroutine huckelip(energy,itype,datacomp)
!------------------------------------
!
! Set ionization potentials
!
...
!---------------------------------------------------------
  subroutine updatemo(cmo,overinv,nproc,myrank,mpi_comm,datacomp)
!---------------------------------------------------------
!
...
!------------------------------------------------------------------------------
  subroutine checkguess(cmoa,cmob,overinv,nproc2,myrank2,mpi_comm2,datacomp)
!------------------------------------------------------------------------------
!
! Read and project MOs
!
...
!
      ...
      implicit none
      type(typecomp),intent(inout) :: datacomp
      integer,intent(in) :: nproc2, myrank2, mpi_comm2
...
!---------------------------------------------------------------------
  subroutine hcoreguess(cmo,h1mtrx,ortho,work,nproc,myrank,mpi_comm,datacomp)
!---------------------------------------------------------------------
!
