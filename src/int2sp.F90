...
!
!-------------------------------------------------------------
  subroutine int2ssss(twoeri,exfac1,exfac2,xyziq,work,nijkl)
!-------------------------------------------------------------
!
...
!       nijkl    (number of primitive pair functions)
! Out : twoeri   (two-electron integral)
!       work     (work space)
!
      ...
      real(8),intent(out) :: twoeri(1), work(2,*)
...
!--------------------------------------------------------------------------------------
  subroutine int2psss(twoeri,exfac1,exfac2,xyziq,xzkl,rot,work,nijkl,maxdim,intorder)
!--------------------------------------------------------------------------------------
!
...
!       nijkl    (number of primitive pair functions)
!       maxdim   (dimension of twoeri array)
!       intorder (order of integral arrangement)
! Out : twoeri   (two-electron integral)
!       work     (work space)
!
      use fmtgrid, only : fgrid, threshtval
      ...
      real(8),intent(out) :: twoeri(maxdim,maxdim,maxdim,maxdim), work(4,*)
      real(8) :: tval, tval2, tval3, tval4, tval5, tval6, tval7, tval8, tval9, tval10
      real(8) :: tinv, ft(0:1)
      real(8) :: f0, f1(2), r0, r1(3)
      real(8) :: ex12, ex34, ex43, ex41,ex14, expq, ex3q, c12, c34, zip, xiq, yiq, ziq
      real(8) :: xypq2, zpq, zpq2, fac, exq, qx, qz, eri(3), eri2(3)
!
! Zero-clear
!
      r0= zero
...
!
      eri2(1)= eri(1)*rot(1,1)+eri(2)*rot(2,1)+eri(3)*rot(3,1)
      eri2(2)= eri(1)*rot(1,2)+eri(2)*rot(2,2)+eri(3)*rot(3,2)
      eri2(3)= eri(1)*rot(1,3)+eri(2)*rot(2,3)+eri(3)*rot(3,3)
!
      select case(intorder)
        case(1,2)
          twoeri(1:3,1,1,1)= eri2(1:3)
        case(3,5)
          twoeri(1,1:3,1,1)= eri2(1:3)
        case(4,7)
          twoeri(1,1,1:3,1)= eri2(1:3)
        case(6,8)
          twoeri(1,1,1,1:3)= eri2(1:3)
      end select
      return
end
...
!---------------------------------------------------------------------------------------
  subroutine int2ppss(twoeri,exfac1,exfac2,xyziq,xzkl,rot,work2,nijkl,maxdim,intorder)
!---------------------------------------------------------------------------------------
!
...
!
      ...
      real(8),parameter :: zero=0.0D+00, one=1.0D+00, sqrtpi4=0.8862269254527580D+00
      real(8),parameter :: two=2.0D+00, three=3.0D+00
      real(8),intent(in) :: exfac1(5,*), exfac2(5,*)
      real(8),intent(in) :: xyziq(3,*), xzkl(2), rot(3,3)
      real(8),intent(out) :: twoeri(maxdim,maxdim,maxdim,maxdim), work2(4,*)
      real(8) :: tval, tval2, tval3, tval4, tval5, tval6, tval7, tval8, tval9, tval10
      real(8) :: tinv, ft(0:2)
      real(8) :: f0, f1(2), f2(3), r0(2), r1(3,2), r2(6)
      real(8) :: ex12, ex34, ex43, ex14, ex41, expq, ex3q, ex4q, c12, c34, zip, xiq, yiq, ziq
      real(8) :: xiq2, yiq2, xypq2, zpq, zpq2, fac, qmd, qmd2, qx, qz, work(3)
      real(8) :: eri(3,3), eri2(3,3)
!
! Zero-clear
!
      r0(1:2)= zero
...
!
      do l= 1,3
        eri2(l,1)= eri(l,1)*rot(1,1)+eri(l,2)*rot(2,1)+eri(l,3)*rot(3,1)
        eri2(l,2)= eri(l,1)*rot(1,2)+eri(l,2)*rot(2,2)+eri(l,3)*rot(3,2)
        eri2(l,3)= eri(l,1)*rot(1,3)+eri(l,2)*rot(2,3)+eri(l,3)*rot(3,3)
      enddo
      do k= 1,3
        eri(1,k)= eri2(1,k)*rot(1,1)+eri2(2,k)*rot(2,1)+eri2(3,k)*rot(3,1)
        eri(2,k)= eri2(1,k)*rot(1,2)+eri2(2,k)*rot(2,2)+eri2(3,k)*rot(3,2)
        eri(3,k)= eri2(1,k)*rot(1,3)+eri2(2,k)*rot(2,3)+eri2(3,k)*rot(3,3)
      enddo
!
      select case(intorder)
        case(1,2)
          do k= 1,3
            do l= 1,3
              twoeri(l,k,1,1)= eri(l,k)
            enddo
          enddo
        case(3,5)
          do k= 1,3
            do l= 1,3
              twoeri(l,k,1,1)= eri(k,l)
            enddo
          enddo
        case(4,7)
          do k= 1,3
            do l= 1,3
              twoeri(1,1,l,k)= eri(l,k)
            enddo
          enddo
        case(6,8)
          do k= 1,3
            do l= 1,3
              twoeri(1,1,l,k)= eri(k,l)
            enddo
          enddo
      end select
      return
...
!--------------------------------------------------------------------------------------
  subroutine int2psps(twoeri,exfac1,exfac2,xyziq,xzkl,rot,work,nijkl,maxdim,intorder)
!--------------------------------------------------------------------------------------
!
...
!
      ...
      real(8),intent(in) :: exfac1(5,*), exfac2(5,*)
      real(8),intent(in) :: xyziq(3,*), xzkl(2), rot(3,3)
      real(8),intent(out) :: twoeri(maxdim,maxdim,maxdim,maxdim), work(6,*)
      real(8) :: tval, tval2, tval3, tval4, tval5, tval6, tval7, tval8, tval9, tval10
      real(8) :: tinv, ft(0:2)
...
!
      select case(intorder)
        case(1)
          do j= 1,3
            do l= 1,3
              twoeri(l,1,j,1)= eri(l,j)
            enddo
          enddo
        case(2)
          do j= 1,3
            do l= 1,3
              twoeri(l,1,1,j)= eri(l,j)
            enddo
          enddo
        case(3)
          do j= 1,3
            do l= 1,3
              twoeri(1,l,j,1)= eri(l,j)
            enddo
          enddo
        case(4)
          do l= 1,3
            do j= 1,3
              twoeri(j,1,l,1)= eri(l,j)
            enddo
          enddo
        case(5)
          do j= 1,3
            do l= 1,3
              twoeri(1,l,1,j)= eri(l,j)
            enddo
          enddo
        case(6)
          do l= 1,3
            do j= 1,3
              twoeri(j,1,1,l)= eri(l,j)
            enddo
          enddo
        case(7)
          do l= 1,3
            do j= 1,3
              twoeri(1,j,l,1)= eri(l,j)
            enddo
          enddo
        case(8)
          do l= 1,3
            do j= 1,3
              twoeri(1,j,1,l)= eri(l,j)
            enddo
          enddo
      end select
      return
...
!---------------------------------------------------------------------------------
  subroutine int2ppps(twoeri,exfac1,exfac2,xyziq,xzkl,rot,nijkl,maxdim,intorder)
!---------------------------------------------------------------------------------
!
...
!
      ...
      real(8) :: qmd, qmd2, qmd2x, qmd2y, qx, qz, xx, xz, zz, work(4), fw(6)
      real(8) :: eri(3,3,3), eri2(3,3,3)
!
! Zero-clear
!
      r0(1:2)= zero
...
!---------------------------------------------------------------------------------
  subroutine int2pppp(twoeri,exfac1,exfac2,xyziq,xzkl,rot,nijkl,maxdim,intorder)
!---------------------------------------------------------------------------------
!
...
!
      ...
      real(8),intent(in) :: xyziq(3,*), xzkl(2), rot(3,3)
      real(8),intent(out) :: twoeri(maxdim,maxdim,maxdim,maxdim)
      real(8) :: r0(4), r1(3,8), r2(6,8), r3(10,4), r4(15), qx, qz, xx, xz, zz
      real(8) :: eri(3,3,3,3), eri2(3,3,3,3)
!
! Zero-clear
!
      r0(1:4)= zero
      ...
      r4(1:15)= zero
!
      call int2pppp2(r0,r1,r2,r3,r4,exfac1,exfac2,xyziq,nijkl)
      qx= xzkl(1)
      qz= xzkl(2)
      xx= qx*qx
      xz= qx*qz
...
!
      do j= 1,3
        do k= 1,3
          do l= 1,3
            eri2(l,k,j,1)= eri(l,k,j,1)*rot(1,1)+eri(l,k,j,2)*rot(2,1)+eri(l,k,j,3)*rot(3,1)
            eri2(l,k,j,2)= eri(l,k,j,1)*rot(1,2)+eri(l,k,j,2)*rot(2,2)+eri(l,k,j,3)*rot(3,2)
            eri2(l,k,j,3)= eri(l,k,j,1)*rot(1,3)+eri(l,k,j,2)*rot(2,3)+eri(l,k,j,3)*rot(3,3)
          enddo
        enddo
      enddo
      do i= 1,3
        do k= 1,3
          do l= 1,3
            eri(l,k,1,i)= eri2(l,k,1,i)*rot(1,1)+eri2(l,k,2,i)*rot(2,1)+eri2(l,k,3,i)*rot(3,1)
            eri(l,k,2,i)= eri2(l,k,1,i)*rot(1,2)+eri2(l,k,2,i)*rot(2,2)+eri2(l,k,3,i)*rot(3,2)
            eri(l,k,3,i)= eri2(l,k,1,i)*rot(1,3)+eri2(l,k,2,i)*rot(2,3)+eri2(l,k,3,i)*rot(3,3)
          enddo
        enddo
      enddo
      do i= 1,3
        do j= 1,3
          do l= 1,3
            eri2(l,1,j,i)= eri(l,1,j,i)*rot(1,1)+eri(l,2,j,i)*rot(2,1)+eri(l,3,j,i)*rot(3,1)
            eri2(l,2,j,i)= eri(l,1,j,i)*rot(1,2)+eri(l,2,j,i)*rot(2,2)+eri(l,3,j,i)*rot(3,2)
            eri2(l,3,j,i)= eri(l,1,j,i)*rot(1,3)+eri(l,2,j,i)*rot(2,3)+eri(l,3,j,i)*rot(3,3)
          enddo
        enddo
      enddo
      do i= 1,3
        do j= 1,3
          do k= 1,3
            eri(1,k,j,i)= eri2(1,k,j,i)*rot(1,1)+eri2(2,k,j,i)*rot(2,1)+eri2(3,k,j,i)*rot(3,1)
            eri(2,k,j,i)= eri2(1,k,j,i)*rot(1,2)+eri2(2,k,j,i)*rot(2,2)+eri2(3,k,j,i)*rot(3,2)
            eri(3,k,j,i)= eri2(1,k,j,i)*rot(1,3)+eri2(2,k,j,i)*rot(2,3)+eri2(3,k,j,i)*rot(3,3)
          enddo
        enddo
      enddo
!
...
!----------------------------------------------------------------
  subroutine int2pppp2(r0,r1,r2,r3,r4,exfac1,exfac2,xyziq,nijkl)
!----------------------------------------------------------------
!
! Calculate (pp|pp) intermidiate integrals
!
! In  : exfac12, exfac34 (exponents and coefficients of primitive pair functions)
!       xyziq    (x,y,z elements of i and q)
!       nijkl    (number of primitive pair functions)
! Out : r0, r1, r2, r3, r4 (intermediate integral)
!
      use fmtgrid, only : fgrid, threshtval
      implicit none
      integer,intent(in) :: nijkl(2)
      ...
      real(8),intent(in) :: exfac1(5,*), exfac2(5,*), xyziq(3,*)
      real(8),intent(inout) :: r0(4), r1(3,8), r2(6,8),r3(10,4), r4(15)
      real(8) :: tval, tval2, tval3, tval4, tval5, tval6, tval7, tval8, tval9, tval10
      real(8) :: tinv, ft(0:4)
      real(8) :: f0(2), f1(2,5), f2(3,5), f3(4,3), f4(5)
      real(8) :: f1w(3,4), f2w(6,5), f3w(10,3), ftw(4,5)
      real(8) :: ex12, ex34, ex43, ex14, ex41, expq, expq2, ex3q, ex4q, c12, c34, zip, zjp
      real(8) :: xiq, yiq, ziq, xiq2, yiq2, xiq4, yiq4, xyiq, xyiq2, xypq2, zpq, zpq2, zpq3
      real(8) :: fac, pmd, qmd, qmd2, qmd2x, qmd2y, qmd2xy, work(5)
!
      do kl= 1,nijkl(2)
        ex34= exfac2(1,kl)
        ex43= exfac2(2,kl)
